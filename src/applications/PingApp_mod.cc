...
#include "PingApp_mod.h"

using std::cout;

Define_Module(PingApp_mod);

simsignal_t PingApp_mod::rttSignal = SIMSIGNAL_NULL;
simsignal_t PingApp_mod::numLostSignal = SIMSIGNAL_NULL;
simsignal_t PingApp_mod::outOfOrderArrivalsSignal = SIMSIGNAL_NULL;
simsignal_t PingApp_mod::pingTxSeqSignal = SIMSIGNAL_NULL;
simsignal_t PingApp_mod::pingRxSeqSignal = SIMSIGNAL_NULL;

void PingApp_mod::initialize(int stage)
{
...
void PingApp_mod::handleMessage(cMessage *msg)
{
...
void PingApp_mod::scheduleNextPing(cMessage *timer)
{
...
void PingApp_mod::sendToICMP(cMessage *msg, const IPvXAddress& destAddr, const IPvXAddress& srcAddr, int hopLimit)
{
    if (!destAddr.isIPv6())
    {
        // send to IPv4
...
void PingApp_mod::processPingResponse(PingPayload *msg)
{
...
void PingApp_mod::countPingResponse(int bytes, long seqNo, simtime_t rtt)
{
...
void PingApp_mod::finish()
{
    ...
    AVG = rttStat.getMean() * 1000.0 ;
    recordScalar("AVG", AVG);
    MIN = rttStat.getMean() * 1000.0 ;
    recordScalar("MIN", MIN);
    MAX = rttStat.getMax() * 1000.0 ;
    recordScalar("MAX", MAX);
