...
//
// modified by Clara Kowald (Fraunhofer ISE)
//
...
#define TRANSMITTERPOWER_DOWN 3     //Clara: Error parameter message
#define TRANSMITTERPOWER_UP 4       //Clara: Error parameter message

simsignal_t Err_Radio::bitrateSignal = registerSignal("bitrate");
simsignal_t Err_Radio::radioStateSignal = registerSignal("radioState");
simsignal_t Err_Radio::channelNumberSignal = registerSignal("channelNo");
simsignal_t Err_Radio::lossRateSignal = registerSignal("lossRate");
simsignal_t Err_Radio::changeLevelNoise = registerSignal("changeLevelNoise");
...
Define_Module(Err_Radio);
Err_Radio::Err_Radio() : rs(this->getId())
{
    obstacles = NULL;
    radioModel = NULL;
    receptionModel = NULL;
...
void Err_Radio::initialize(int stage)
{
    ...
    else if (stage == 2)
    {
        ...
        //Clara: Error parameters:

        int interval = par("changetransmitterPowerInterval");

        if(interval > 0){
            simtime_t d = simTime() + (simtime_t)par("changetransmitterPowerInterval");
            timermsg = new cMessage("changetransmitterPower");
            timermsg->setKind(TRANSMITTERPOWER_DOWN);
            scheduleAt(d, timermsg);
        }
    }
}

void Err_Radio::finish()
{
...
bool Err_Radio::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
bool Err_Radio::processAirFrame(AirFrame *airframe)
{
 ...
 * handleSelfMsg
 */
void Err_Radio::handleMessage(cMessage *msg)
{
 ...
 */
void Err_Radio::bufferMsg(AirFrame *airframe) //FIXME: add explicit simtime_t atTime arg?
{
...
AirFrame *Err_Radio::encapsulatePacket(cPacket *frame)
{
...
void Err_Radio::sendUp(AirFrame *airframe)
{
...
void Err_Radio::sendDown(AirFrame *airframe)
{
 ...
 */
AirFrame *Err_Radio::unbufferMsg(cMessage *msg)
{
 ...
 * considered as noise.
 */
void Err_Radio::handleUpperMsg(AirFrame *airframe)
{
...
void Err_Radio::handleCommand(int msgkind, cObject *ctrl)
{
...
void Err_Radio::handleSelfMsg(cMessage *msg)
{
    ...
    else if(msg->getKind() == TRANSMITTERPOWER_DOWN)
    {
        //set transmitterPower to minimal transmitterPower
        transmitterPower = par("min_transmitterPower");

        //schedule new self message to change bitrate to normal bitrate
        simtime_t d = simTime() + (simtime_t)par("changetransmitterPowerInterval");

        cancelEvent(timermsg);
        timermsg->setKind(TRANSMITTERPOWER_UP);
        scheduleAt(d, timermsg);

    }
    else if(msg->getKind() == TRANSMITTERPOWER_UP)
    {
        //set bitrate to normal bitrate
        transmitterPower = par("transmitterPower");

        //schedule new self message to change bitrate to minimal bitrate
        simtime_t d = simTime() + (simtime_t)par("changetransmitterPowerInterval");
        cancelEvent(timermsg);
        timermsg->setKind(TRANSMITTERPOWER_DOWN);
        scheduleAt(d, timermsg);
 ...
 */
void Err_Radio::handleLowerMsgStart(AirFrame* airframe)
{
 ...
 */
void Err_Radio::handleLowerMsgEnd(AirFrame * airframe)
{
...
void Err_Radio::changeChannel(int channel)
{
...
void Err_Radio::setBitrate(double bitrate)
{
...
void Err_Radio::setRadioState(RadioState::State newState)
{
...
void Err_Radio::updateSensitivity(double rate)
{
...
void Err_Radio::registerBattery()
{
...
void Err_Radio::updateDisplayString() {
...
double Err_Radio::calcDistFreeSpace()
{
...
double Err_Radio::calcDistDoubleRay()
{
    //the carrier frequency used
    //minimum power level to be able to physically receive a signal
    double minReceivePower = sensitivity;
...
void Err_Radio::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj)
{
...
void Err_Radio::disconnectReceiver()
{
...
void Err_Radio::connectReceiver()
{
    receiverConnected = true;
...
void Err_Radio::getSensitivityList(cXMLElement* xmlConfig)
{
